...
- State schema: `RecipeWorkflowState(MessagesState)` with fields for user context, pantry data, recipe results, and response. `user_preferences` dict holds `allergies`, `restrictions`, `cuisines`, `excluded_food_types`, `diet`, and `skill`.
...
- `recipe_knowledge_agent.py` — hybrid vector+keyword search using fastembed (`sentence-transformers/all-MiniLM-L6-v2`, dim=384) against Milvus/Zilliz Cloud; falls back to local Qdrant. `hybrid_query` accepts `allergies`, `preferred_cuisines`, and `excluded_food_types` parameters. Post-filters results to exclude allergen-containing recipes and recipes matching excluded food type categories (dessert, soup, appetizer, etc.) via title-keyword matching using the `FOOD_TYPE_KEYWORDS` class dict. Uses adaptive `allow_missing` (starts at 0, escalates to the requested limit) and requires `num_pantry_used >= 1` to prevent irrelevant recipes from appearing.
