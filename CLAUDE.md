...
- Fallback: local Qdrant (`./qdrant_data/`, `qdrant-client` + `sentence-transformers`). Activated automatically when Zilliz is unavailable. Uses `data/ingredient_index.json` for pantry candidate lookup and Qdrant vector search for semantic similarity.
